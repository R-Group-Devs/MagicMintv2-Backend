...
exports.getAllClaimsByUser = async (req, res) =>  {
    ...
    campaignIDs.forEach(async (campaignID) => {
        ...
        const campaignData = await axios.get(`https://api.twitter.com/2/tweets/${campaignID}/retweeted_by`, config)
        ...
        if(campaignData.data.meta.result_count  == 0) {
           ...
           reshares.forEach(async (reshare, index) =>{  
                ...
                if(reshare.username == req.params.user){
                    ...
                    let specificCampaign  = await Campaign.findOne({}).where("twitterPostID").equals(campaignID)

                    let specificNFTMetadata = await CampaignNFT.find({}).where("file").equals(specificCampaign.campaignNFTID)

                    fs.readFile(path.join(__dirname,`../public/assets/upload/NFTPrototype${specificCampaign.campaignNFTID}`), async (err, data) => {
                        if(err){
                            throw err;
                        }else{
...
exports.getClaimedNFTs = async (req, res) => {
    ...
    let claims  = await ClaimNFT.find({}).where("owner").equals(req.params.user).where("isMinted").equals(true)
